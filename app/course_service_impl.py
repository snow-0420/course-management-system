...
class CourseServiceImpl(CourseService):
        ...
        for course in self.course_lst:
            if course_name == course.name:
                raise InstanceExistError(
                    "Course with {0} has already existed".format(course_name))
    ...
    def create_assignment(self, course_id, assignment_name):
        ...
        for assign in course.assignments:
            if assignment_name == assign.name:
                raise InstanceExistError(
                    "{0} already has an assignment with name {1}".format(
                        course.name, assignment_name))
        self.assignment_service.create_assignment(assignment_name,
                                                               course)
    ...
    def enroll_student(self, course_id, student_id):
        ...
        if student_id in course.enrolled_students:
            raise InstanceExistError(
                "Student {0} is already enrolled in course {1}".format(
    ...
    def dropout_student(self, course_id, student_id):
        ...
        if student_id not in course.enrolled_students:
            raise InstanceNotFoundError(
                "Student {0} is not enrolled in course {1}".format(student_id,
                                                                   course_id))
        student = self.student_service.get_student_by_id(student_id)
    ...
    def get_top_five_students(self, course_id) -> List[int]:
        ...
        grades = [(student_id, self.get_student_grade_avg(course_id, student_id)) for student_id in course.enrolled_students]
        grades.sort(key=lambda pair: pair[1], reverse=True) # sort using avg_grade in the tuple

        # return top 5 (or all if less than 5 students)
        return [pair[0] for pair in (grades[:5] if len(grades) >= 5 else grades)]
